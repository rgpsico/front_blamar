<template>
  <div class="activities-manager">
    <div class="activities-manager__header">
      <div>
        <h2>Activities</h2>
        <p>Cadastro completo de atividades do Incentive usando a API api_atividades_incentive.php.</p>
      </div>
      <v-spacer></v-spacer>
      <v-btn color="primary" class="mr-2" @click="openCreate">Nova Activity</v-btn>
      <v-btn outlined color="primary" @click="fetchActivities">Atualizar</v-btn>
    </div>

    <v-card class="activities-manager__filters" elevation="4">
      <v-row>
        <v-col cols="12" md="4">
      ...
      <div class="activities-manager__filter-actions">
    ...
    <v-card elevation="6">
      <v-data-table :headers="headers" :items="items" :loading="loading" item-key="id" class="elevation-0">
        ...
        <template slot="item.tags" slot-scope="{ item }">
          <div class="activities-manager__chips">
    ...
    <v-dialog v-model="dialog" max-width="1100px" persistent>
      <v-card>
        <v-card-title class="activities-manager__dialog-title">
          <div>
            <div>{{ dialogTitle }}</div>
            <div class="activities-manager__dialog-subtitle">{{ editedItem.slug || 'Preencha os dados da activity' }}</div>
          </div>
          <v-spacer></v-spacer>
            ...
            <v-tabs-items v-model="activeTab" class="mt-4">
              <v-tab-item>
                <v-row>
                  ...
                  <v-col cols="12" md="2">
                    <v-text-field v-model.number="editedItem.capacidade_min" label="Capacidade min" type="number" outlined dense></v-text-field>
                  </v-col>
                  <v-col cols="12" md="2">
                    <v-text-field v-model.number="editedItem.capacidade_max" label="Capacidade max" type="number" outlined dense></v-text-field>
                  </v-col>
                  <v-col cols="12" md="3"><v-switch v-model="editedItem.is_active" label="Ativo" inset></v-switch></v-col>
                  <v-col cols="12" md="3"><v-switch v-model="editedItem.is_classic" label="Classic" inset></v-switch></v-col>
                  <v-col cols="12" md="3"><v-switch v-model="editedItem.is_favourite" label="Favourite" inset></v-switch></v-col>
                  <v-col cols="12" md="3"><v-switch v-model="editedItem.is_out_of_box" label="Out of box" inset></v-switch></v-col>
                  <v-col cols="12">
              ...
              <v-tab-item>
                <v-row>
                  ...
                  <v-col cols="12" md="4">
                    <v-text-field :value="editedItem.price_range_label" label="Faixa exibida" outlined dense readonly></v-text-field>
                  </v-col>
                  <v-col cols="12">
              ...
              <v-tab-item>
                <div class="activities-manager__tab-head">
                  <div>Fotos da activity</div>
                  <v-btn small outlined color="primary" @click="addPhoto">Adicionar foto</v-btn>
                </div>
                <v-row v-for="(foto, index) in editedItem.fotos" :key="foto.id || `new-photo-${index}`" class="activities-manager__photo-row">
                  <v-col cols="12" md="6">
                  ...
                  <v-col cols="12" md="4">
                    <v-img v-if="previewPhotoUrl(foto.url)" :src="previewPhotoUrl(foto.url)" height="120" cover class="activities-manager__photo-preview"></v-img>
                  </v-col>
                </v-row>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-alert type="warning" border="left" colored-border>
            Tem certeza que deseja excluir a activity
...
const IMAGE_BASE = 'http://www.blumar.com.br/global/main_site/images/incentive_activities/'
...
export default {
  ...
  computed: {
    dialogTitle() {
      return this.editedIndex === -1 ? 'Nova Activity' : 'Editar Activity'
    },
    citySelectItems() {
      return (this.cities || [])
        .map((city) => ({
  ...
  watch: {
    'editedItem.price_range'(value) {
      const amount = Number(value)
      this.editedItem.price_range_label = Number.isFinite(amount) && amount > 0 ? '$'.repeat(amount) : ''
    },
  ...
  methods: {
      ...
      return {
        ...
        cidade_id:
          base.cidade_id !== null && base.cidade_id !== undefined && `${base.cidade_id}` !== ''
            ? Number(base.cidade_id)
            : null,
        capacidade_min: this.toIntOrNull(base.capacidade_min),
        capacidade_max: this.toIntOrNull(base.capacidade_max),
    ...
    buildQuery() {
      ...
      params.append('request', 'listar_activities')
    ...
    async fetchCities() {
      try {
        const response = await fetch(`${API_BASE}api_atividades_incentive.php?request=listar_cidades`, {
    ...
    async fetchActivities() {
      this.loading = true
      try {
        const response = await fetch(`${API_BASE}api_atividades_incentive.php?${this.buildQuery()}`, {
          headers: this.authHeaders()
        })
        const data = await response.json()
        ...
        this.items = data.map(this.normalizeActivity)
      } catch (error) {
        this.showMessage(`Erro ao carregar activities: ${error.message}`, 'error')
      } finally {
        this.loading = false
      }
    },
    ...
    async openEdit(item) {
      ...
      try {
        const response = await fetch(
          `${API_BASE}api_atividades_incentive.php?request=buscar_activity&id=${item.id}`,
          { headers: this.authHeaders() }
        )
        const data = await response.json()
        if (data?.error) {
        ...
        const normalized = this.normalizeActivity(data)
        this.editedItem = normalized
        this.originalPhotos = JSON.parse(JSON.stringify(normalized.fotos || []))
      } catch (error) {
        this.showMessage(`Erro ao carregar activity: ${error.message}`, 'error')
      } finally {
    ...
    openDelete(item) {
      this.editedIndex = this.items.findIndex((entry) => entry.id === item.id)
      this.editedItem = this.normalizeActivity(item)
    ...
    buildActivityPayload() {
      return {
        cidade_id: this.toIntOrNull(this.editedItem.cidade_id),
        ...
        capacidade_min: this.toIntOrNull(this.editedItem.capacidade_min),
        capacidade_max: this.toIntOrNull(this.editedItem.capacidade_max),
    ...
    async syncPhotos(activityId) {
      ...
      for (const foto of removedPhotos) {
        await this.requestJson(`${API_BASE}api_atividades_incentive.php?request=excluir_foto&id=${foto.id}`, {
          method: 'DELETE',
          headers: this.authHeaders()
        })
      }
      ...
      for (const foto of currentPhotos) {
        ...
        if (foto.id) {
          ...
          await this.requestJson(`${API_BASE}api_atividades_incentive.php?request=atualizar_foto&id=${foto.id}`, {
            method: 'PUT',
            headers: {
        ...
        await this.requestJson(`${API_BASE}api_atividades_incentive.php?request=adicionar_foto&activity_id=${activityId}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            ...this.authHeaders()
    ...
    async save() {
      if (!String(this.editedItem.nome || '').trim()) {
        this.showMessage('Informe o nome da activity.', 'warning')
        return
      }
      ...
      try {
        ...
        const url = isEdit
          ? `${API_BASE}api_atividades_incentive.php?request=atualizar_activity&id=${this.editedItem.id}`
          : `${API_BASE}api_atividades_incentive.php?request=criar_activity`
        ...
        const activityId = isEdit ? this.editedItem.id : result.activity_id
        await this.syncPhotos(activityId)
        this.showMessage(isEdit ? 'Activity atualizada.' : 'Activity criada.')
        this.dialog = false
    ...
    async confirmDelete() {
      ...
      this.saving = true
      try {
        await this.requestJson(`${API_BASE}api_atividades_incentive.php?request=excluir_activity&id=${this.editedItem.id}`, {
          method: 'DELETE',
          headers: this.authHeaders()
        })
        this.showMessage('Activity excluida.')
...
.activities-manager__header h2 {
  margin: 0;
  font-size: 24px;
}

.activities-manager__header p {
  margin: 4px 0 0;
  color: #64748b;
  font-size: 13px;
}

.activities-manager__filters {
  padding: 16px;
  margin-bottom: 16px;
}

.activities-manager__filter-actions {
  display: flex;
  gap: 8px;
  margin-top: 4px;
}

.activities-manager__dialog-title {
  display: flex;
  align-items: center;
}

.activities-manager__dialog-subtitle {
  font-size: 12px;
  color: rgba(0, 0, 0, 0.6);
}

.activities-manager__chips {
  display: flex;
  flex-wrap: wrap;
  gap: 4px;
}

.activities-manager__tab-head {
  display: flex;
  align-items: center;
...
.activities-manager__photo-preview {
  border-radius: 10px;
  overflow: hidden;
}
</style>
