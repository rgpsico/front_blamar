...
// Busca nome e cidade da activity se ainda não foram definidos
if (!isset($activity_name) || $activity_name === '') {
    $id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
    if ($id > 0) {
        $sql = "
            SELECT a.nome, c.nome_cid AS cidade_nome
        ...
        if ($res && pg_num_rows($res) > 0) {
            ...
            $activity_name = $row['nome']       ?? '';
            if (!isset($city_name) || $city_name === '') {
                $city_name = $row['cidade_nome'] ?? '';
            }
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .language-toggle {
            ...
            border-radius: 4px; padding: 4px 6px; height: 30px;
            cursor: pointer; display: inline-flex; align-items: center; gap: 6px;
        }
        .language-toggle .material-icons { font-size: 16px; color: #666; }
        .language-menu {
            position: absolute; top: 34px; left: 0; display: none;
            background: #fff; border: 1px solid #d0d0d0; border-radius: 6px;
            padding: 6px; z-index: 20; box-shadow: 0 6px 18px rgba(0,0,0,0.12);
        }
        .language-menu.is-open { display: flex; flex-direction: column; gap: 6px; }
        .language-option {
            border: 1px solid transparent; background: transparent;
            padding: 2px; border-radius: 4px; cursor: pointer;
        }
        .language-option:hover, .language-option.is-active { border-color: #e69928; }
    ...
    <title><?php echo isset($activity_name) && $activity_name !== '' ? h($activity_name) : 'Activity'; ?></title>
</head>
<body>
...
<header>
    <div class="container">
        <div class="logo_topo">
            <img src="<?php echo h(asset_url('img/logo_blumar.png')); ?>" alt="Blumar">
        </div>
        <div class="menu_interno">
            ...
            <!-- Botão volta para listagem de activities -->
            <a href="<?php echo h(app_url('pages/activities/activities_list.php')); ?>">
                <button type="button" style="cursor:pointer;">Back to Activities</button>
            </a>
        </div>
    </div>
</header>
    ...
    <div class="container_max ct02">
        <div class="container">
            <div class="breadcrumb">
                <a href="<?php echo h(app_url('index.php')); ?>">Incentive Area</a>
                <i class="material-icons">&#xe315;</i>
                <a href="<?php echo h(app_url('pages/activities/activities_list.php')); ?>">Activities</a>
                <i class="material-icons">&#xe315;</i>
                <a href=""><?php echo isset($city_name) && $city_name !== '' ? h($city_name) : 'City'; ?></a>
                <i class="material-icons">&#xe315;</i>
                <a href=""><?php echo isset($activity_name) && $activity_name !== '' ? h($activity_name) : 'Activity'; ?></a>
            </div>
        </div>
    </div>
</section>
...
<script>
    ...
    function updateLanguageUi() {
        ...
        if (flag && flagMap[current]) flag.src = flagMap[current];
    ...
    function hideGoogleIframes() {
        ['.goog-te-banner-frame', '.goog-te-menu-frame', 'iframe.skiptranslate'].forEach(function(sel) {
