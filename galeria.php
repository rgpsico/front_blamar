...
switch ($action) {
    ...
    case 'tours_by_city':
        if (!isset($_GET['cidade_cod']) || empty(trim($_GET['cidade_cod']))) {
            http_response_code(400);
            echo json_encode(['error' => 'cidade_cod obrigatÃ³rio']);
            exit;
        }

        $cidade_cod = pg_escape_string(trim($_GET['cidade_cod']));

        // Busca tpocidcod da cidade
        $query_cid = "SELECT tpocidcod
        FROM tarifario.cidade_tpo
        WHERE cidade_cod = '$cidade_cod'
        LIMIT 1";

        $result_cid = pg_exec($conn, $query_cid);
        if (!$result_cid || pg_numrows($result_cid) == 0) {
            http_response_code(404);
            echo json_encode(['error' => 'Cidade nÃ£o encontrada']);
            exit;
        }

        $fk_tpocidcod = pg_result($result_cid, 0, 'tpocidcod');

        $pega_tours = "SELECT pk_descritivo_tours, nome_en
        FROM tarifario.descritivo_tours
        WHERE fk_tpocidcod = '$fk_tpocidcod'
        ...
        $tours = [];
        for ($row = 0; $row < pg_numrows($result_tours); $row++) {
            $tours[] = [
                'pk_descritivo_tours' => pg_result($result_tours, $row, 'pk_descritivo_tours'),
                'nome_en' => pg_result($result_tours, $row, 'nome_en'),
                'cidade_cod' => $cidade_cod
            ];
        }

        echo json_encode([
            'action' => 'tours_by_city',
            'total' => count($tours),
            'cidade_cod' => $cidade_cod,
            'tours' => $tours
        ]);
    ...
    case 'hotels_by_city':
        ...
        if (isset($_GET['cidade_cod']) && !empty(trim($_GET['cidade_cod']))) {
            $fk_cidcod = pg_escape_string(trim($_GET['cidade_cod']));
        } else {
            if (!isset($_GET['city']) || empty(trim($_GET['city']))) {
                http_response_code(400);
                echo json_encode(['error' => 'Parametro "city" ou "cidade_cod" e obrigatorio']);
                exit;
            }
        ...
        LIMIT 1";
            ...
            if (!$result_cidade || pg_numrows($result_cidade) == 0) {
                http_response_code(404);
                echo json_encode(['error' => 'Cidade nao encontrada']);
                exit;
            }
